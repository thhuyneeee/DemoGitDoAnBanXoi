        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            if (KiemTra())
            {
                using (var dbContext = new QLBXDBcontext())
                {
                    ...
                    int index = dgvDS.Rows.Add();
                    dgvDS.Rows[index].Cells[0].Value = txtMaXoi.Text.ToString();
                    dgvDS.Rows[index].Cells[1].Value = txtTenXoi.Text.ToString();
                    dgvDS.Rows[index].Cells[2].Value = txtGia.Text.ToString();
                    CaiDatMacDinh();
                    btnLuu.Enabled = true;
                    btnKhongLuu.Enabled = true;
